...
export class PrismaRepository implements Repository {
 ...
 private async load(tx:Tx):Promise<Data>{
  ...
  return {
   users:users.map(u=>({...u,disabledAt:ms(u.disabledAt),totpEnabledAt:ms(u.totpEnabledAt)})),partners:partners.map(p=>({...p,contractStartedAt:ms(p.contractStartedAt)})),venues:venues.map(v=>({...v,branding:v.branding as unknown as VenueBranding|null})),
 ...
 private async save(tx:Tx,before:Data,after:Data){
  ...
  await sync(before.users,after.users,u=>{const data={...u,disabledAt:date(u.disabledAt),authVersion:u.authVersion??0,totpSecret:u.totpSecret??null,totpEnabledAt:date(u.totpEnabledAt),totpLastStep:u.totpLastStep??null};return tx.user.upsert({where:{id:u.id},create:data,update:data});});
  ...
  await sync(before.manufacturerSyncRuns,after.manufacturerSyncRuns,row=>{const data={...row,errorSummary:row.errorSummary as unknown as Prisma.InputJsonValue,startedAt:new Date(row.startedAt),completedAt:date(row.completedAt)};return tx.manufacturerSyncRun.upsert({where:{id:row.id},create:data,update:data});},ids=>tx.manufacturerSyncRun.deleteMany({where:{id:{in:ids}}}));
