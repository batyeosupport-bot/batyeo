...
### C. Comptes et accès (petits, mais bloquants à l'usage)
- **Aucun changement de mot de passe en libre-service.** Un partenaire qui perd le sien doit être recréé par un admin. `settings` ne permet que de changer son nom affiché.
- **Aucun bouton pour désactiver un compte** — mais `disabledAt` **est** bien appliqué à la connexion (`core/security.ts:24`) : seule la commande manque, pas la sécurité.
- Aucune gestion d'équipe (ajouter un second utilisateur à un partenaire existant). Les utilisateurs ne viennent que de `seedData` ou, désormais, de `partner/create`.

### D. Panneau d'état du système (déjà proposé, jamais fait)
Afficher dans l'admin le mode de paiement, le fournisseur matériel et le verrou d'éjection — aujourd'hui lisibles uniquement via `/api/core/health` en JSON brut. L'incident `DATABASE_URL` aurait été diagnostiqué en secondes.
