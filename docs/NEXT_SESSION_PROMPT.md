...
> **CRITIQUE (2026-09-21) : le parcours client ne collecte aucune carte.** `StripePaymentProvider.authorize` crée un PaymentIntent sans jamais y attacher de moyen de paiement, et l'ancien code le marquait « autorisé » sans lire son statut. `stripe_live` est désormais refusé tant que l'autorisation n'est pas `requires_capture` (`requireConfirmedAuthorization`), donc **le mode réel est bloqué jusqu'à la construction de la collecte de carte** : Stripe Payment Element côté téléphone, nouvelle route qui vérifie l'intent côté serveur avant d'éjecter, expiration des locations jamais confirmées. C'est le premier chantier à faire.
