...
function provider(opts:{authorizeFail?:boolean;captureFail?:boolean;releaseFail?:boolean}={}){
 const calls:StripeRequest[]=[];const stripe=new StripePaymentProvider('sk_test_coordinator',async request=>{calls.push(request);if(opts.authorizeFail&&request.path==='/payment_intents')throw new Error('authorization declined');if(opts.captureFail&&request.path.includes('/capture'))throw new Error('capture timeout');if(opts.releaseFail&&request.path.includes('/cancel'))throw new Error('release timeout');return {id:'pi_'+(request.path.includes('/capture')?'capture':'auth'),status:'requires_capture',amount:Number(request.body.amount??2000),amount_received:0};});return {stripe,calls};
}
...
test('An ejection failure whose subsequent Stripe release also fails surfaces as ERROR, never a silent EJECTION_FAILED hiding an unreleased deposit',async()=>{const repo=new MemoryRepository(seedData('unused'));const station=new MockBatteryStationProvider();station.simulateFailure(repo.data,'station-paris','ejection');const {stripe}=provider({releaseFail:true});const coordinator=new StripeRentalCoordinator(stripe,station);const rental=await coordinator.start(repo,'stripe-double-failure','station-paris','stripe-double-fail',1_800_000_000_000);assert.equal(rental.state,'ERROR');assert.notEqual(rental.state,'EJECTION_FAILED');assert.equal(repo.data.payments.find(p=>p.rentalId===rental.id)?.status,'UNKNOWN');assert.ok(evaluateAlerts(repo.data,1_800_000_001_000).some(a=>a.kind==='PAYMENT_MISMATCH'&&a.rentalId===rental.id));});
