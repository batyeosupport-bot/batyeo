...
test('a sync takes the cabinet as the truth for a linked station: online follows the read, business data does not',async()=>{const data=seedData('x');data.stations[0].online=false;linkManufacturerStation(data,'station-paris','BAJIE','BJH02347',100);const repo=new MemoryRepository(data),service=new ManufacturerSyncService(repo,{getDeviceInfo:async id=>snapshot(id,true)},{now:()=>200});const run=await service.run({trigger:'MANUAL'});assert.equal(run.status,'COMPLETED');assert.equal(repo.data.stations[0].online,true,'the cabinet said online, so the station is sellable again');assert.equal(repo.data.stations[0].providerStatus,'ONLINE');assert.equal(repo.data.stationProviderSnapshots[0].busySlots,99);assert.ok(repo.data.reconciliationRecords.some(row=>row.kind==='STATION_STATUS'&&row.status==='OPEN'));assert.ok(repo.data.reconciliationRecords.some(row=>row.kind==='UNKNOWN_BATTERY'));});
