...
export function createMedia(d:Data,input:CreateMediaInput):StationMedia {
 ...
 const media:StationMedia={id:crypto.randomUUID(),name:input.name.trim(),kind:input.kind,uri:input.uri,checksum:crypto.randomUUID().replace(/-/g,''),durationMs:input.durationMs,status:'DRAFT',startsAt:input.startsAt??null,endsAt:input.endsAt??null,targetStationIds:input.targetStationIds??[],createdAt:Date.now(),updatedAt:Date.now()};
