...
export function validateData(d:Data):void {
 ...
 for(const s of d.stations){ensure(d.venues.some(v=>v.id===s.venueId&&v.partnerId===s.partnerId),'station tenant');ensure(integer(s.capacity,1),'station capacity');if(s.provider)ensure(['mock','manufacturer'].includes(s.provider),'station provider');if(s.providerLastSyncedAt!=null)ensure(integer(s.providerLastSyncedAt),'manufacturer sync timestamp');if(s.lastSeenAt!=null)ensure(integer(s.lastSeenAt),'manufacturer last seen timestamp');}
