...
export function validateData(d:Data):void {
 ...
 for(const p of d.payments){ensure(d.rentals.some(r=>r.id===p.rentalId),'orphan payment');ensure(PAYMENT_STATES.includes(p.status),'payment state');ensure([p.authorizedCents,p.capturedCents,p.releasedCents,p.refundedCents??0].every(n=>integer(n))&&p.capturedCents+p.releasedCents<=p.authorizedCents,'money conservation');ensure((p.refundedCents??0)<=p.capturedCents,'refund never exceeds what was captured');if(['CAPTURED','RELEASED'].includes(p.status))ensure(p.capturedCents+p.releasedCents===p.authorizedCents,'settled authorization');if(p.status==='RELEASED')ensure(p.capturedCents===0,'released capture');if(p.requestedCents!==undefined)ensure(integer(p.requestedCents,1),'requested payment amount');}
 for(const b of d.batteries){const slots=d.slots.filter(s=>s.batteryId===b.id);ensure(integer(b.charge)&&b.charge<=100,'battery charge');ensure(b.status==='RENTED'||b.status==='LOST'||b.status==='MISSING'?slots.length===0:slots.length===1,'battery location');if(b.status==='RENTED')ensure(d.rentals.some(r=>r.batteryId===b.id&&OPEN_STATES.includes(r.state)),'rented battery without rental');if(b.status==='LOST')ensure(d.rentals.some(r=>r.batteryId===b.id&&r.state==='LOST'),'lost battery without rental');}
